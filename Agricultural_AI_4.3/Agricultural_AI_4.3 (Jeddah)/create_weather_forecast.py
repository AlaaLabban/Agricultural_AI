...
class WeatherForecaster:
    def __init__(self, historical_data_path='90-92 vars(in).csv'):
    ...
    def create_features(self, df):
        """
        --- MODIFIED: Creates both time-based and lag/rolling features ---
        """
        df = df.copy()
        # Time-based features
        ...
        # Rolling average features (average over the last week)
    ...
    def create_forecast(self):
        ...
        # --- MODIFIED: Split dates along with the data to make them available for plotting ---
        ...
        for target in target_variables:
            ...
            if target == 'precipitation_sum':
                ...
                rainy_days = self.df[self.df['precipitation_sum'] > 0]
                avg_rain_days_per_year = len(rainy_days) / len(self.df['date'].dt.year.unique())
                ...
                # --- MODIFIED: Pass the correct dates_test variable to the plotting function ---
                ...
                for date in future_dates:
                    ...
                    new_row = {'date': date}
                    # Add all target predictions to the history for the next iteration's lag features
                    for col in target_variables:
                        if col == target:
                            new_row[col] = prediction
                        elif col in forecast_df.columns:
                            # Use already predicted values for other variables if available
                            new_row[col] = forecast_df.loc[forecast_df['date'] == date, col].iloc[0] if not \
                            forecast_df.loc[forecast_df['date'] == date].empty else 0
                        else:
                            new_row[col] = 0  # Placeholder if not predicted yet
